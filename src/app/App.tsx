...
function AdminChat({ courses, students }: { courses: Course[]; students: Profile[] }) {
  ...
  useEffect(() => {
    if (isMobile && chatType === "course" && courses.length > 0 && !selectedCourseId) {
      setSelectedCourseId(courses[0].id);
      setShowList(false);
    }
    if (isMobile && chatType === "personal" && students.length > 0 && !selectedStudentId) {
