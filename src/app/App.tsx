...
// ─── Student Module Viewer (FIXED - with proper module unlocking) ──────────────
...
function StudentModuleViewer({ profile, enrollment, modules, moduleContents, onNavigate, onProgressUpdate }: { 
  ...
  useEffect(() => {
    if (enrollment) {
      // Refresh enrollment data periodically to check for updates
  ...
  const handleModuleSelect = (index: number) => {
    ...
    if (index <= currentIndex + 1) {
      ...
      if (index === currentIndex + 1) {
        // This is the next module - check if previous is passed
  ...
  const isModuleLocked = (index: number) => {
    ...
    // If index is greater than current_index + 1, it's locked
  ...
  const handleSubmitQuiz = async () => {
    ...
    try {
      ...
      if (score >= (quizData.pass_score || 70)) {
        ...
        // Refresh enrollment data to get updated module index
        await fetchEnrollmentData();
        
        // Refresh progress data
        await fetchProgress();
        
        // Check if the module was passed and enrollment was updated
      ...
      <div className="w-full md:w-64 bg-muted/30 border-b md:border-b-0 md:border-r border-border p-4 shrink-0 overflow-y-auto max-h-[calc(100vh-80px)]">
        ...
        <div className="space-y-1">
          {modules?.map((module, index) => {
            ...
            const isCurrentOrPrevious = index <= (currentEnrollment?.current_module_index || 0);
            ...
            return (
              ...
              >
                ...
                {isCurrentOrPrevious && !isCompleted && !isActive && !isNext && (
                  <Badge variant="muted" className="text-[10px] px-1.5 py-0.5">📝</Badge>
                )}
                {prog?.status === "failed" && (
        ...
        <div className="mt-3 text-xs text-muted-foreground border-t border-border pt-3 space-y-1">
          ...
          {currentEnrollment && (
            <p className="text-xs text-muted-foreground">
              Current: Module {currentEnrollment.current_module_index + 1}
            </p>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 p-4 md:p-6 overflow-y-auto max-h-[calc(100vh-80px)]">
        <div className="mb-4">
          <div className="flex flex-wrap items-center gap-2 mb-1">
            ...
            {isAdvancing && <Badge variant="warning">⏳ Loading next module...</Badge>}
          </div>
                        ...
                        onClick={() => {
                          ...
                          if (nextIndex < modules.length) {
                            ...
                            if (!nextProgress || nextProgress.status === "locked") {
                              // Module is locked, but we just passed the current one
                              // Wait for the enrollment to update and then try again
                              const checkAndAdvance = async () => {
                                await fetchEnrollmentData();
                                await fetchProgress();
                                // Check if enrollment has been updated
